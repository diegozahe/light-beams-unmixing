...
def entropy(d):
	...
	for x in d:
		if x != 0:
			en += x/(1.*total) * np.log(x/(1.*total))/math.log(2.0)
	return -en
...
def comprobarMancha(array, mancha):
	...
	for c in array:
		...
		residuoImage = ImageChops.difference(b, d)
		residuoNP = np.array(residuoImage)
		histograma = cv2.calcHist([residuoNP],[0],None,[256],[0,256])
		entropia = entropy(histograma) #entropia = st.entropy(histograma)
		print("ha calculado la entropia", entropia)
		mediaEntropia.append(entropia)
	masPequenio = 1000
	if not mediaEntropia:
		pass
	else:
...
#camara = cv2.VideoCapture("out.mp4")
#*****************
#INICIALIZAMOS!!!!
#*****************
